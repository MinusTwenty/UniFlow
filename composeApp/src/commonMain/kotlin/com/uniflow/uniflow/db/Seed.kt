...
fun seedUser1FirstSemester(db: UniFlowDatabase) {
    val user1 = db.usersQueries.getUserByUsername("user1").executeAsOneOrNull() ?: return
    ...
    fun ensureCourse(code: String, name: String, credits: Int = 0) =
    ...
    fun enroll(courseId: Long) {
        ...
        try {
            db.enrollmentsQueries.enroll(user1.id, term.id, courseId)
        } catch (_: Throwable) { }
    }
    ...
    // Hétfő (1)
    db.lessonsQueries.insertLesson(mit.id, term.id, 1, "13:15", "14:00", "EVERY", "P", "PRACTICE", g312.id, paksi.id, validFrom, validTo, null)
    db.lessonsQueries.insertLesson(phw.id, term.id, 1, "16:40", "17:25", "EVERY", "C", "LECTURE", inf01.id, marak.id, validFrom, validTo, null)

    // Kedd (2)
    db.lessonsQueries.insertLesson(pr1.id, term.id, 2, "08:00", "09:30", "EVERY", "C", "LECTURE", info3.id, vegh.id, validFrom, validTo, null)
    db.lessonsQueries.insertLesson(spr.id, term.id, 2, "09:40", "11:10", "EVERY", "C", "SEMINAR", inf01.id, csoka.id, validFrom, validTo, null)

    // Szerda (3)
    db.lessonsQueries.insertLesson(udi.id, term.id, 3, "09:40", "11:00", "EVERY", "", "OTHER", k102k2.id, csoka.id, validFrom, validTo, "Egyes alkalmak külön dátumosak lehetnek.")
    db.lessonsQueries.insertLesson(udi.id, term.id, 3, "13:15", "14:45", "EVERY", "S", "SEMINAR", g312.id, csoka.id, validFrom, validTo, null)
    db.lessonsQueries.insertLesson(ma1.id, term.id, 3, "16:50", "17:35", "EVERY", "C", "LECTURE", dp002.id, arki.id, validFrom, validTo, null)
    db.lessonsQueries.insertLesson(ma1.id, term.id, 3, "17:35", "18:20", "EVERY", "S", "SEMINAR", dp002.id, arki.id, validFrom, validTo, null)

    // Csütörtök (4)
    db.lessonsQueries.insertLesson(phw.id, term.id, 4, "08:00", "09:30", "EVERY", "A", "LECTURE", dp005.id, molnar.id, validFrom, validTo, null)
    db.lessonsQueries.insertLesson(ma1.id, term.id, 4, "09:40", "11:10", "EVERY", "P", "PRACTICE", dp005.id, szalay.id, validFrom, validTo, null)
    db.lessonsQueries.insertLesson(sport.id, term.id, 4, "11:20", "12:05", "EVERY", "C", "SPORT", null, bognar.id, validFrom, validTo, null)
    db.lessonsQueries.insertLesson(tex.id, term.id, 4, "13:15", "14:45", "EVERY", "C", "LECTURE", k106.id, vontszemu.id, validFrom, validTo, null)
    db.lessonsQueries.insertLesson(udi.id, term.id, 4, "17:40", "18:25", "EVERY", "P", "PRACTICE", k101k1.id, kiss.id, validFrom, validTo, null)

    // Péntek (5)
    db.lessonsQueries.insertLesson(mit.id, term.id, 5, "13:05", "13:50", "EVERY", "P", "PRACTICE", k102k2.id, takac.id, validFrom, validTo, null)

    // PR1 pénteki gyakorlat: csak bizonyos dátumokon (occurrences)
    db.lessonsQueries.insertLesson(pr1.id, term.id, 5, "14:00", "17:54", "EVERY", "P", "PRACTICE", k103k3.id, kato.id, validFrom, validTo, "Csak adott dátumokon (lesson_occurrences).")
    ...
    if (pr1Friday != null) {
        val dates = listOf("2023-09-22", "2023-09-29", "2023-10-06", "2023-10-27", "2023-11-24", "2023-12-08")
        for (d in dates) {
            try { db.lesson_occurrencesQueries.insertOccurrence(pr1Friday.id, d) } catch (_: Throwable) { }
        }
    }
}